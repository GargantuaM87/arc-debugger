#ifndef ADB_PARSE_HPP
#define ADB_PARSE_HPP
...
namespace adb {
    ...
    std::optional<I> to_integral(std::string_view sv, int base = 10) { // return optional<I> which reprents either I or nothing
        ...
        I ret;
        auto result = std::from_chars(begin, sv.end(), ret, base); // parse the string into ret

        if(result.ptr != sv.end()) {
            return std::nullopt; // if there's still some characters to be parsed, then return this
        }
        return ret;
    }
    ...
    auto parse_vector(std::string_view text) {
        ...
        for(auto i = 0; i < N - 1; ++i) {
            // parse the first four characters from c into a byte (first four characters being an integer)
            bytes[i] = to_integral<std::byte>({c, 4}, 16).value();
            c += 4; // skip c over the integer we just parsed
            if(*c++ != ',') // make sure there's a comma
                invalid();
        }
