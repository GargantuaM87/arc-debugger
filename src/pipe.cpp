...
adb::pipe::pipe(bool close_on_exec) {
    if(pipe2(fds_, close_on_exec ? O_CLOEXEC : 0) < 0) { // pipe2 used to set flags while creating a pipe
        error::send_errno("Pipe creation failed");
    }
}

adb::pipe::~pipe() { //RAII standard
    close_read();
    close_write();
}
...
std::vector<std::byte> adb::pipe::read() {
    char buf[1024];
    int chars_read;

    if((chars_read = ::read(fds_[read_fd], buf, sizeof(buf))) < 0) { //fill the buffer from the read end
...
void adb::pipe::write(std::byte* from, std::size_t bytes) {
